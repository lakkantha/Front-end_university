...
@Component({
  selector: 'app-final-marks-approval-new',
  templateUrl: './final-marks-approval-new.component.html',
  styleUrls: ['./final-marks-approval-new.component.css']
})
export class FinalMarksApprovalNewComponent implements OnInit {
  ...
  Programs = []; GenProName = 'Entry Exam Name'; academicCenter = []; medium = []; centreId = 0; adminDetailId = 0;
  ...
  constructor(private fb: FormBuilder, private api: FinalMarkApproveService, private spinner: NgxSpinnerService) {
    this.mainForm = this.fb.group({
      ...
      all: new FormControl('', [Validators.nullValidator, Validators.required]),
  ...
  ngOnInit(): void {
    this.mainForm.get("entryExamName").patchValue("Entry Exam Name");
    ...
    this.mainForm.get("all").patchValue("notAll");
    document.getElementById("notAll").click();
  }
  ...
  changePreferd(event) {
    if (event.target.checked) {
      if (event.target.value == "allselected") {
        (document.getElementById("academicCentre") as HTMLInputElement).disabled = true;
        (document.getElementById("mediumread") as HTMLInputElement).disabled = true;
        this.selectMedium(0);
      } else {
        (document.getElementById("academicCentre") as HTMLInputElement).disabled = false;
        (document.getElementById("mediumread") as HTMLInputElement).disabled = false;
      }
    }
  }
